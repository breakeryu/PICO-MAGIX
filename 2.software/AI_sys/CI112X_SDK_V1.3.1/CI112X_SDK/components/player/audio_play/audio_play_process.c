...
/**************************************************************************
                     define
****************************************************************************/
#define READBUF_SIZE            (4000U) 
#define MAX_OUTPUTBUFF_SIZE     (2304U)
#define MIN_OUTPUTBUFF_SIZE     (576U)
#define PCM_BUF_COUNT           (AUDIO_PLAY_BLOCK_CONT) //(AUDIO_PLAY_BLOCK_CONT*(MAX_OUTPUTBUFF_SIZE)/(MIN_OUTPUTBUFF_SIZE))
...
static __no_init uint16_t pcm_buf_no_cache[MIN_OUTPUTBUFF_SIZE*PCM_BUF_COUNT];
#endif
...
static int32_t task_audio_play_init_step(void)
{
    ...
    memset(pcm_buf_base,0,sizeof(uint16_t)*MIN_OUTPUTBUFF_SIZE*PCM_BUF_COUNT);
#else
...
static int32_t find_decoder_ops(const char* decoder_name)
{
    ...
    {
        if(0 ==  strcmp(curr_decoder_ops->decoder_name, decoder_name))
        {
            /* 配置解码器模式 */
    ...
    if((0 ==  strcmp("history", decoder_name))&&(history_decoder_ops != NULL))
    {
    ...
    for(;ops != NULL;ops = ops->next_ops)
    {
        if(0 == strcmp(ops->decoder_name, decoder_name))
        {
...
{  
    memset(decbuf,0,MIN_OUTPUTBUFF_SIZE*sizeof(uint16_t));
...
static int32_t audio_play_decode_continue(void)
{
    ...
    while(--get_frame_flag_timeout > 0)
    {
        ...
        {
            curr_decoder_ops->decode(data_in_buffer, data_in_size, (short*)decbuf, &audio_play_output_size, &no_use_size, &audio_format_info);
            play_will_finish_curr_play_size += audio_play_output_size;
            decode_size += data_in_size - no_use_size;
            data_in_size = no_use_size;
            curr_decoder_ops->decode(data_in_buffer + decode_size, data_in_size, (short*)decbuf, &audio_play_output_size, &no_use_size, &audio_format_info);
            play_will_finish_curr_play_size += audio_play_output_size;
            decode_size += data_in_size - no_use_size;
            data_in_size = no_use_size;
            memmove(data_in_buffer, data_in_buffer + decode_size, data_in_size);
            audio_read_offset = decode_size;
        ...
        if(0 ==  curr_decoder_ops->decode(data_in_buffer,data_in_size,(short*)decbuf,&audio_play_output_size,&no_use_size,&audio_format_info))
        {
            ...
            /* 启动声卡开始播放 */
            if (decode_frame_count == 2)
            {
                ci_logdebug(LOG_AUDIO_PLAY, "play hw start\n");
                audio_play_hw_start(DISABLE);
            }
        ...
        {
            valid_data = valid_data - decode_size;
        }
        else
        {
         ...
         */
        /* 待解码buf数据补充 */
        if(no_use_size < curr_decoder_ops->data_in_size)
        {
            /* 拷贝剩余数据到buf起始位置 */
            memmove(data_in_buffer,data_in_buffer+data_in_size-no_use_size,no_use_size);
        ...
        {
            /* 从循环缓冲buf读取数据 */
            if(GET_PLAY_DATA_BUFF_SIZE-temp_wave_read_size >= curr_decoder_ops->data_in_size-no_use_size)
            {
                /* 当前循环缓冲buf数据补充至待解码buf，达到data_in_size大小 */
                memcpy(data_in_buffer+no_use_size,(uint8_t *)(wav_buf+temp_wave_read_size),curr_decoder_ops->data_in_size-no_use_size);
            }
            else
            {
                /* 当前循环缓冲buf数据不足补充到data_in_size大小 */
                memcpy(data_in_buffer+no_use_size,(uint8_t *)(wav_buf+temp_wave_read_size),GET_PLAY_DATA_BUFF_SIZE-temp_wave_read_size);
                no_use_size += GET_PLAY_DATA_BUFF_SIZE-temp_wave_read_size;
                temp_wave_read_size = 0;

                if(AUDIO_PLAY_FLAG_WILL_END == play_end_flag)
                {
                    memset(data_in_buffer+no_use_size,0x0,curr_decoder_ops->data_in_size-no_use_size);
            ...
            temp_wave_read_size += curr_decoder_ops->data_in_size-no_use_size;
            no_use_size=curr_decoder_ops->data_in_size;
...
static int32_t audio_play_start(void)
{
    ...
    if(play_end_flag == AUDIO_PLAY_FLAG_RESTART)
    {
        ...
        {
            memcpy(data_in_buffer+start_offset,(uint8_t *)(wav_buf+temp_wave_read_size),data_in_size-start_offset);
...
static int32_t parse_m4a(uint32_t bak_data_size)
{
    ...
    if(0 != strncmp((char*)(bak_data+ftpy_size+4),"moov",4))
    {
    ...
    while((m4a_size < moov_size) && (play_end_flag == AUDIO_PLAY_FLAG_RESTART))
    {
        ...
        if(rev_flag == AUDIO_DATD_BUFF_END)
        {
            memcpy(bak_data+m4a_size,(uint8_t *)wav_buf_addr,rev_size);
            m4a_size += rev_size;
            ci_logdebug(LOG_AUDIO_PLAY,"this is end buf!\n");
            play_end_flag = AUDIO_PLAY_FLAG_DATA_END;
            break;
        ...
        {
            memcpy(bak_data+m4a_size,(uint8_t *)wav_buf_addr,GET_PLAY_DATA_BUFF_SIZE);
    ...
    if(m4a_file_info.aac_stco_struct->mdatOffset >=  m4a_size)
    {
        ...
        while((mdat_size < rel_size) && (play_end_flag == AUDIO_PLAY_FLAG_RESTART))
        {
            ...
            if(rev_flag == AUDIO_DATD_BUFF_END)
            {
                memcpy(bak_data+mdat_size,(uint8_t *)wav_buf_addr,rev_size);
                mdat_size += rev_size;
                ci_logdebug(LOG_AUDIO_PLAY,"this is end buf!\n");
                play_end_flag = AUDIO_PLAY_FLAG_DATA_END;
                break;
            ...
            {
                memcpy(bak_data+mdat_size,(uint8_t *)wav_buf_addr,GET_PLAY_DATA_BUFF_SIZE);
...
static int32_t parse_mp3(uint32_t bak_data_size)
{
    ...
    else
    {
        ...
        while((mp3_size < id3_size) && (play_end_flag == AUDIO_PLAY_FLAG_RESTART))
        {
            ...
            if(rev_flag == AUDIO_DATD_BUFF_END)
            {
                memcpy(bak_data,(uint8_t *)wav_buf_addr,rev_size);
                mp3_size += rev_size;
                ci_logdebug(LOG_AUDIO_PLAY,"this is end buf!\n");
                play_end_flag = AUDIO_PLAY_FLAG_DATA_END;
                break;
            ...
            {
                memcpy(bak_data,(uint8_t *)wav_buf_addr,GET_PLAY_DATA_BUFF_SIZE);
...
static int32_t parse_file(audio_play_msg_t *play_msg)
{
    ...
    memcpy(bak_data,(uint8_t *)wav_buf_addr,data_size);
    if(rev_flag == AUDIO_DATD_BUFF_END)
    {
        ci_logdebug(LOG_AUDIO_PLAY,"this is end buf!\n");
        play_end_flag = AUDIO_PLAY_FLAG_DATA_END;
    ...
    if(strncmp((char*)bak_data,"RIFF",4)  == 0)
    {
        /* MS Wav 文件 */
        /* 目前不需要特殊处理，跳过44字节头即可 */
    ...
    else if (strncmp((char*)(bak_data+4),"ftyp",4)  == 0)
    {
        /* m4a文件 */
    ...
    else if (strncmp((char*)(bak_data),"ID3",3)  == 0)
    {
        /* mp3文件 */
    ...
    else if (strncmp((char*)bak_data,"fLaC",4)  == 0)
    {
        /* flac文件 */
        bak_data_fp = mf_open(bak_data,data_size,NULL);
    ...
    for(;;)
    {
        switch(audio_play_state)
        {
            ...
            case AUDIO_PLAY_STATE_START:
            {
                ...
                #if AUDIO_PLAYER_CONFIG_AUTO_PARSE_AUDIO_FILE
                if((play_msg.play_data_type != AUDIO_PLAY_DATA_TYPE_CONTINUE)&&(0 !=  strcmp("history", play_msg.decoder_name)))
                {
                    /* 处理文件头信息 */
            ...
            case AUDIO_PLAY_STATE_STOP:
            {
                ...
                else
                {
                    ...
                    memset(decbuf,0x0,audio_play_output_size);
                    audio_play_hw_write_data(decbuf,audio_play_output_size);
                ...
                /*清除解码缓存*/
                memset(data_in_buffer,0,READBUF_SIZE);
