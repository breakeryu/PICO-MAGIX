...
static void welcome(void)
{
    ...
    ci_loginfo(LOG_USER,"\033[1;32mWelcome to ci112x_sdk.\033[0;39m\r\n");
}
...
/**
 * @brief 任务间通信变量的初始化，例如,消息队列、信号量等的创建、驱动包含os相关外设初始化
 * 
 */
static void vTaskVariablesInit(void)
{
	...
	if(1 == flash_check_mode(QSPI0))
	{
		ci_loginfo(LOG_USER,"flash current mode is quad!!!\n");
	}
	else
	{
		ci_loginfo(LOG_USER,"flash current mode is stand!!!\n");
	}
...
static void vTaskCreate (void *p_arg)
{
    ...
    /* 用户任务 */
    xTaskCreate(UserTaskManageProcess,"UserTaskManageProcess",320,NULL,4,NULL);

    /* ASR系统启动任务 */
	xTaskCreate(asr_system_startup_task,"asr_system_startup_task",512,NULL,4,NULL);

    /* 语音输入任务 */
    xTaskCreate(audio_in_preprocess_mode_task,"audio_in_preprocess",512,&audio_in_preprocess_mode_id,4,&audio_in_preprocess_mode_handle);
...
int main(void)
{
    ...
    /* 创建启动任务 */
    xTaskCreate(vTaskCreate,"task creat",480,NULL,4,NULL);
